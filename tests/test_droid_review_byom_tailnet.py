...
def _get_byom_step() -> dict:
    ...
    for step in shard_job.get("steps", []):
        if step.get("name") == BYOM_STEP_NAME:
            return step
    pytest.fail(f"review-shard job must have a {BYOM_STEP_NAME!r} step")


def _get_step_comment_block(step_name: str) -> str:
    ...
    assert step_lines, f"step {step_name!r} not found in workflow source"
    comments: list[str] = []
...
def _extract_settings_block() -> str:
    """从 run: 块提取 heredoc 内嵌的 settings JSON 原文。"""
    run_script = _get_byom_step()["run"]
...
def _load_settings() -> dict:
    ...
    return json.loads(_extract_settings_block())


class TestByomTailnetRouting:
    """BYOM baseUrl 必须走 ts 内网直达 Kong，禁止回退公网端点。"""

    def test_baseurl_points_to_tailnet_kong(self):
    ...
    def test_heredoc_delimiter_quoted(self):
    ...
    def test_hosts_prerequisite_documented(self):
        ...
        # 也可能出现在 run: 内 —— 两者合并检查。
        comments = _get_step_comment_block(BYOM_STEP_NAME)
        combined = comments + "\n" + run_script
